...
export function Aside() {
  ...
  return (
    ...
    >
      {/* Avatar yang mengarah ke Home */}
      <Link href="/">
        <Atoms.Avatar
          src={MY_PROFILE.avatar}
          className={cn(
            'size-20 transition-all duration-700 cursor-pointer',
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          {/* Nama yang mengarah ke Home */}
          <Link href="/" className="font-semibold text-lg hover:text-primary transition-colors duration-200">
            {MY_PROFILE.name}
          </Link>
          <Atoms.Verified />
        </div>
      ...
      <nav className="grid gap-1">
        {NAV_MENUS.sort((a, b) => a.sort - b.sort).map((item) => (
          <Atoms.Button
            id={item.label}
            key={item.href}
            size="lg"
